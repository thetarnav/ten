...
export const node_var_encode = (lhs: Node_Id, rhs: Ident_Id): Node_Key => {
...
export const node_var = (ctx: Context, lhs: Node_Id, rhs: Ident_Id): Node_Id => {
...
export const world_fork = (ctx: Context, id: Node_Id, body: Node_Id): Node_Id => {
    let node = node_world(ctx, body)
    let world = world_get_assert(ctx, node)
    world.parent = id
    return node
}
...
const node_from_expr = (ctx: Context, world_id: Node_Id, expr: Expr, src: string, scope_id: Node_Id): Node_Id | null => {
    ...
    case EXPR_TOKEN:
        ...
        case TOKEN_IDENT: {
            ...
            return node_var(ctx, NODE_ID_NONE, ident)
        }
        }
        return null
    ...
    case EXPR_BINARY: {
        ...
        case TOKEN_OR: {
            ...
            if (lhs_existing != null) {
                world_alias(ctx, lhs_existing, lhs_world_id, lhs)
            ...
            if (rhs_existing != null) {
                world_alias(ctx, rhs_existing, rhs_world_id, rhs)
                rhs_world_id = rhs_existing
            } else {
                ctx.nodes.set(rhs_key, rhs_world_id)
            }

            let lhs_world = world_get_assert(ctx, lhs_world_id)
            let rhs_world = world_get_assert(ctx, rhs_world_id)
            lhs_world.parent = world_id
            rhs_world.parent = world_id
    ...
    case EXPR_PAREN: {
        ...
        if (expr.open.kind === TOKEN_BRACE_L) {
            ...
            // scope id is needed to be generated before body is processed
            ...
            let existing = ctx.nodes.get(key)
            if (existing != null) {
                scope_vars_alias(ctx, world_id, existing, scope_id)
                return existing
            }
            ctx.nodes.set(key, scope_id)
            return scope_id
        }
        // Regular paren (...)
...
const scope_vars_alias = (ctx: Context, world_id: Node_Id, dst_id: Node_Id, src_id: Node_Id): void => {
    if (dst_id === src_id) return

    let world = world_get_assert(ctx, world_id)
    let dst = world.vars.get(dst_id)
    let src = world.vars.get(src_id)

    if (dst == null && src == null) return
    if (dst == null && src != null) {
        world.vars.set(dst_id, src)
        world.vars.set(src_id, src)
        return
    }
    if (dst != null && src == null) {
        world.vars.set(src_id, dst)
        return
    }
    if (dst != null && src != null && dst !== src) {
        for (let [ident, val] of src) {
            dst.set(ident, val)
        }
        world.vars.set(src_id, dst)
    }
}
const world_alias = (ctx: Context, dst_id: Node_Id, src_id: Node_Id, body_id: Node_Id): void => {
    if (dst_id === src_id) return

    let dst_node = node_by_id(ctx, dst_id)
    let src_node = node_by_id(ctx, src_id)
    if (dst_node == null || src_node == null) return
    if (dst_node.kind !== NODE_WORLD || src_node.kind !== NODE_WORLD) return

    for (let [scope_id, var_map] of src_node.world.vars) {
        let canon_scope = scope_id
        let dst_map = dst_node.world.vars.get(canon_scope)
        if (!dst_map) {
            dst_node.world.vars.set(canon_scope, new Map(var_map))
        } else {
            for (let [ident, val] of var_map) {
                dst_map.set(ident, val)
            }
        }
    }

    if (dst_node.world.parent == null) {
        dst_node.world.parent = src_node.world.parent
    }

    dst_node.body = body_id
    src_node.body = body_id
    src_node.world = dst_node.world
}
...
const node_reduce = (ctx: Context, node_id: Node_Id, world_id: Node_Id, scope_id: Node_Id, visited: Set<Node_Id>): Node_Id => {
    ...
    case NODE_SCOPE: {
        ...
        if (body.kind === NODE_OR) {
            let lhs_scope = node_scope(ctx, body.lhs)
            let rhs_scope = node_scope(ctx, body.rhs)
            world.vars.set(lhs_scope, new Map(vars))
            world.vars.set(rhs_scope, new Map(vars))
            return node_reduce(ctx, node_or(ctx,
                lhs_scope,
                rhs_scope,
            ), world_id, scope_id, visited)
    ...
    case NODE_WORLD: {
        ...
        node.body = body_id
        ctx.nodes.set(node_world_encode(body_id), node_id)
        return node_id
    }
